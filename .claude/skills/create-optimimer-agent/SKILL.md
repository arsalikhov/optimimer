...
| `jev`       | `label`, `kind` (choice/noul/score), `state` (text, or JSON; empty = `{{input}}`), `instructions`, `options` (choice: one per line `name: meaning`, include a way out like `other`), `levels` (score: 2–10 lines, lowest first), `when_true`/`when_false` (noul), `min_confidence` (default 0.6) | `{{id.answer}}`, `{{id.confidence}}`, `{{id.confident}}` (+ `{{id.probabilities}}`); `{{id.enabled}}` is false without `JEV_API_KEY` |
